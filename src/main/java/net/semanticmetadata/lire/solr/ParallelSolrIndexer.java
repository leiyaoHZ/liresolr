			...
			while (!locallyEnded) {
				...
				try {
					if (!locallyEnded) {
						...
						sb.append("file:" + tmp.getFileName());
						sb.append("</field>");
						...
						if (!individualFiles) {
							...
							String filePath = "/tmp" + tmp.getFileName() + "_solr.xml";
							System.out.println("Setting out file to " + filePath);
							OutputStream mos = new BufferedOutputStream(new FileOutputStream(filePath));
